...
type source = Uint8ClampedArray;

type teacher = {
  id: number;
  canTeach: Array<course>;
};
...
type student = {
  id: number;
  requires: Array<course>;
  wants: Array<course>;
};

type utils = {
  bytesPerTeacher: number;
  bytesPerStudent: number;
  bytesPerCourse: number;
...
class DNABuilder {
  constructor(data: data, utils: utils) {
    this.data = data;
    this.utils = utils;
  }
  getByteType(index) {
  ...
  build() {
    ...
    return new DNA(
      { beginning: 0, length: this.utils.bytesPerDNA },
      arr,
      this.utils,
      this.data
    );
...
class Slice {
  constructor(range: range, source: source, utils: utils, data: data) {
    this.range = range;
    this.source = source;
    this.utils = utils;
    this.data = data;
...
class DNA extends Slice {
  ...
  score() {
    ...
    let expectedNumberOfEmptySlots =
    ...
    for (
      let periodNum = 0;
      periodNum < this.data.meta.numPeriodsAvailable;
      periodNum++
    ) {
      let arr = [];
      ...
      for (
        let roomNum = 0;
        roomNum < this.data.meta.numRoomsAvailable;
        roomNum++
      ) {
        let word = this.parseInt(
          roomNum * this.utils.bytesPerRow +
...
class PopulationManager {
  ...
  createPopulation(number: number) {
